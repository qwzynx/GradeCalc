    ...
    <GlassCard className="p-4 border-black/10 shadow-sm bg-white">
      <form onSubmit={onSubmit} className="flex flex-col gap-3">
                ...
                <div className="flex gap-3 items-end w-full mt-4">
                  ...
                  <div className="flex flex-col gap-1 shrink-0 bg-black/5 border border-black/10 rounded p-1 mb-[2px]">
                    <button 
                      ...
                      className={`px-3 py-2 rounded text-[10px] sm:text-xs uppercase tracking-widest transition-all min-w-[80px] ${(!inputModes[i] || inputModes[i] === "percentage") ? 'bg-primary text-white font-bold shadow-sm' : 'text-muted hover:text-secondary'}`}
                    >
                      % Match
                    </button>
                    <button 
                      ...
                      className={`px-3 py-2 rounded text-[10px] sm:text-xs uppercase tracking-widest transition-all min-w-[80px] ${inputModes[i] === "points" ? 'bg-primary text-white font-bold shadow-sm' : 'text-muted hover:text-secondary'}`}
                    >
                      Points
                    </button>
                  </div>
          ...
          <div className="flex gap-2 mt-2">
              ...
              {editingAssignment && (
                  <button type="button" onClick={() => onDelete(editingAssignment)} className="px-4 min-w-[44px] min-h-[44px] flex items-center justify-center bg-white border border-red-500 text-red-500 rounded hover:bg-red-50 hover:text-red-600 transition-colors shadow-sm">
